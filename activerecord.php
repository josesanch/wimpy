...
//				return $this->$property;
			...
			if($field['l10n'] && web::instance()->l10n->isNotDefault($selected_lang)) {
				...
				if($this->row_data_l10n[$selected_lang][$property]) return $this->row_data_l10n[$selected_lang][$property];
...
//																		and field='".$property."'
				...
				if(!$sta || $sta->rowCount() == 0) {
					return $return_default_lang_is_not_exists ? $this->row_data[$property] : null;
				}
				...
				if($rows = $sta->fetchAll()) {
					...
					return $this->row_data_l10n[$selected_lang][$property];
				}
			}
			return $this->row_data[$property];
		}
	}
