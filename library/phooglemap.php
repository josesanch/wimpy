...
*               (thus eliminating the need for two different show methods from version 1.0)
*/
	function addAddress($address,$htmlMessage = null, $icon = null){
...
*/
	...
	public function addCustomIcon($name, $url) {
		$icon = new GIcon($name);
...
//    		  		map = new GMap2(document.getElementById(\"map\"));\n";
		...
		if ($this->showControl) {
			if ($this->controlType == 'small') { $str.= "map.addControl(new GSmallMapControl());\n"; }
          	if ($this->controlType == 'large') { $str.= "map.addControl(new GLargeMapControl());\n"; }
		}

		if ($this->showType) {
			$str.= "map.addControl(new GMapTypeControl());\n";
		}
		foreach($this->icons as $name => $icon) {
			$str .= "var icon_$name = new GIcon(G_DEFAULT_ICON);\n";
			foreach(get_object_vars($icon) as $item => $value) {
				if($value) $str .= "icon_$name.$item = $value;\n";
			}
		}
	    $numPoints = count($this->validPoints);
    	for ($g = 0; $g < $numPoints; $g++) {
    		if($this->validPoints[$g]['icon']) {
    			$str .= "var options".$g." = { icon: icon_".$this->validPoints[$g]['icon']." };\n";
    		} else {
	    		$str .= "var options".$g." = {};\n";
    		}

    	    $str.= "point".$g." = new GPoint(".$this->validPoints[$g]['long'].",".$this->validPoints[$g]['lat'].");\n
					marker".$g." = new GMarker(point".$g.", options".$g.");\n
		            map.addOverlay(marker".$g.")\n
        		    GEvent.addListener(marker".$g.", \"click\", function() {\n";

			$htmlMessage = $this->validPoints[$g]['htmlMessage']!=null ? $this->validPoints[$g]['htmlMessage'] : $this->validPoints[$g]['passedAddress'];
			if($this->validPoints[$g]['callback']) {
				$str.= "if(".$this->validPoints[$g]['callback']."(marker".$g.")) ";
			}
       		$str.= "marker".$g.".openInfoWindowHtml(\"".$htmlMessage."\");\n";
