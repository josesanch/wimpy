...
class helpers_files extends ActiveRecord
{
    ...
    public function url($short = false)
    {
        if ($short) return $this->path."/".$this->id.".".$this->extension;
    ...
    public function getType() { $explode = explode('/', $this->tipo); return array_shift($explode);	}
    public function getSubtype() { $explode = explode('/', $this->tipo); return array_pop($explode);	}
