...
function GridFiles(field, model, vid, vtmp_upload) {
    ...
    this.load = function() {
        ...
        $('#container-files-' + fieldName).load(
            "/ajax/" + modelName + "/files/read/" + fieldName + "/" + id + "/?tmp_upload=" + tmp_upload,
            function(data) {
                ...
                $('#container-files-' + fieldName +' a.images-delete').bind('click', function (e) {
                    ...
                    if (confirm('¿Está seguro de querer realizar esta operación?')) {
                        $.get("/ajax/" + tmodel + '/files/destroy/' + tfield + "/" + tid  + "?tmp_upload=" + ttmp_upload, function() {
                            self.load();
                        });
                    }
                    return false;
    ...
    $("#uploadify_" + fieldName).uploadify({
        ...
        'script'        : "/ajax/" + modelName + "/files/save/" + fieldName + "/" + id,
