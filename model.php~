...
*/
	...
	public function __get($item) {
		if($this->has_files && $item == "files") {
			...
			return $images->select("module='".get_class($this)."' and iditem='".$this->$primary_key."' and field=''", "order:orden");
		}
	...
	**********************************************************************************************/
	...
	public function listImagesAjax($id) {
			...
			$module = web::request("tmp_upload") ? web::request("tmp_upload") : get_class($this);
...
//		log::to_file("no existe fichero ".$file['tmp_name']);
	...
	public function saveFromRequest() {
		...
		if(web::request("tmp_upload")) {
			...
			$this->database->exec("UPDATE images set iditem='$item->id',module='$module' where module='".web::request("tmp_upload")."'");
		}
	}
